...
class MultiLayerGuardrailSystem:
    ...
    def layer3_phi_detection(self, user_input: str) -> ValidationResult:
        """
    ...
    def layer4_terminology_validation(self, text: str, enforce: bool = False) -> ValidationResult:
        """
    ...
    def process_with_guardrails(
        ...
        # LAYER 3: PHI Detection
        result = self.layer3_phi_detection(user_input)
        validation_log.append(result)
        if not result.passed:
        ...
        # LAYER 4: Medical Terminology Validation
        result = self.layer4_terminology_validation(output, enforce=False)
        validation_log.append(result)
        if not result.passed:
