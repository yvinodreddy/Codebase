...
class MedicalCodeDatabase:
    ...
    def search_icd10(self, query: str, max_results: int = 10, min_confidence: float = 0.5) -> List[Tuple[str, ICD10Code, float]]:
        ...
        Supports both exact and partial keyword matching
        ...
        for code, code_obj in self.icd10_codes.items():
            ...
            if confidence == 0:
                for keyword in code_obj.keywords:
                    normalized_kw = self._normalize_text(keyword)
                    # Partial match: query is substring of keyword
                    if normalized_query in normalized_kw:
                        # Calculate partial confidence based on match quality
                        confidence = len(normalized_query) / len(normalized_kw)
                        # Boost if query matches complete word in keyword
                        if normalized_query in normalized_kw.split():
