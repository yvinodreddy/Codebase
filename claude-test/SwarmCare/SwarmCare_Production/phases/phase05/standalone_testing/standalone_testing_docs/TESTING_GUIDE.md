# Testing Guide - Phase 05: Knowledge Graph
...
# 4. Open browser to http://localhost:8005
# 5. Verify UI loads
...
**Expected Output:**
```
🚀 Starting Phase 05: Knowledge Graph Testing
...
🌐 Access UI at: http://localhost:8005
```
...
#### Access Points
1. **Main UI:** http://localhost:8005
2. **API Docs:** http://localhost:8005/docs
3. **OpenAPI:** http://localhost:8005/openapi.json
...
```bash
# Get requirements
curl http://localhost:8005/api/requirements | python3 -m json.tool

# Update requirements (example)
curl -X POST http://localhost:8005/api/requirements \
...
# Get results
curl http://localhost:8005/api/test/results | python3 -m json.tool
```

#### Data Management
```bash
# Check data status
curl http://localhost:8005/api/data/status | python3 -m json.tool

# Reseed data
curl -X POST http://localhost:8005/api/data/seed
```
...
time curl http://localhost:8005/api/health
...
hey -n 100 -c 10 http://localhost:8005/api/health
```
...
curl http://localhost:8005/api/database/status

# Test query
curl -X POST http://localhost:8005/api/database/query \
    -H "Content-Type: application/json" \
...
curl http://localhost:8005/api/cache/status

# Test cache operations
curl -X POST http://localhost:8005/api/cache/set \
    -H "Content-Type: application/json" \
    -d '{"key": "test", "value": "data"}'

curl http://localhost:8005/api/cache/get/test
```
...
curl http://localhost:8005/api/test/report > test_report.json
