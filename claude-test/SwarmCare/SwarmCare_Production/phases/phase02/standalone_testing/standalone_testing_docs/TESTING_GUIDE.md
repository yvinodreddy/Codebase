# Testing Guide - Phase 02: NLP Processing
...
**Expected Output:**
```
🚀 Starting Phase 02: NLP Processing Testing
...
#### Access Points
1. **Main UI:** http://localhost:8002
2. **API Docs:** http://localhost:8002/docs
3. **OpenAPI:** http://localhost:8002/openapi.json
...
```bash
# Get requirements
curl http://localhost:8002/api/requirements | python3 -m json.tool

# Update requirements (example)
curl -X POST http://localhost:8002/api/requirements \
...
# Get results
curl http://localhost:8002/api/test/results | python3 -m json.tool
```

#### Data Management
```bash
# Check data status
curl http://localhost:8002/api/data/status | python3 -m json.tool

# Reseed data
curl -X POST http://localhost:8002/api/data/seed
```
...
time curl http://localhost:8002/api/health
...
hey -n 100 -c 10 http://localhost:8002/api/health
```
...
curl http://localhost:8002/api/database/status

# Test query
curl -X POST http://localhost:8002/api/database/query \
    -H "Content-Type: application/json" \
...
curl -X POST http://localhost:8002/api/cache/set \
    -H "Content-Type: application/json" \
    -d '{"key": "test", "value": "data"}'

curl http://localhost:8002/api/cache/get/test
```
...
curl http://localhost:8002/api/test/report > test_report.json
