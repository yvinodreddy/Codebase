# Testing Guide - Phase 07: FHIR Integration
...
# 4. Open browser to http://localhost:8007
# 5. Verify UI loads
...
**Expected Output:**
```
🚀 Starting Phase 07: FHIR Integration Testing
...
🌐 Access UI at: http://localhost:8007
```
...
#### Access Points
1. **Main UI:** http://localhost:8007
2. **API Docs:** http://localhost:8007/docs
3. **OpenAPI:** http://localhost:8007/openapi.json
...
```bash
# Get requirements
curl http://localhost:8007/api/requirements | python3 -m json.tool

# Update requirements (example)
curl -X POST http://localhost:8007/api/requirements \
...
```bash
# Run tests
curl -X POST http://localhost:8007/api/test/run \
    -H "Content-Type: application/json" \
    -d '{"test_suite": "all"}'

# Get results
curl http://localhost:8007/api/test/results | python3 -m json.tool
```

#### Data Management
```bash
# Check data status
curl http://localhost:8007/api/data/status | python3 -m json.tool

# Reseed data
curl -X POST http://localhost:8007/api/data/seed
```
...
time curl http://localhost:8007/api/health
...
hey -n 100 -c 10 http://localhost:8007/api/health
```
...
curl http://localhost:8007/api/database/status

# Test query
curl -X POST http://localhost:8007/api/database/query \
    -H "Content-Type: application/json" \
...
curl http://localhost:8007/api/cache/status

# Test cache operations
curl -X POST http://localhost:8007/api/cache/set \
    -H "Content-Type: application/json" \
    -d '{"key": "test", "value": "data"}'

curl http://localhost:8007/api/cache/get/test
```
...
curl http://localhost:8007/api/test/report > test_report.json
