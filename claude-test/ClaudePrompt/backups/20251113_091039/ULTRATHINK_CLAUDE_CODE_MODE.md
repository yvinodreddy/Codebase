...
- ✅ Quality scoring (99%+ target)
- ✅ Iterative refinement protocol
- ✅ Context management
- ✅ Production-ready validation

**AND:**
- ✅ No API charges (uses $200/month Claude Code subscription)
- ✅ Full file access (read, write, modify, execute)
- ✅ Same as typing to Claude Code prompt
- ✅ But with ULTRATHINK quality enhancements
...
When you run:
```bash
cpp "generate images from latex paper"
```
...
**When you run:**
```bash
cpp "prompt"          # Default mode - NO API
cpp "prompt" -q       # Quiet mode - NO API
cpp "prompt" --verbose # Verbose mode - NO API
```

**These modes:**
- ✅ Use Claude Code (included in $200/month subscription)
...
```bash
cpp "prompt" --api    # API mode - YES, charges API fees
```

**API mode:**
- ❌ Charges $0.003-0.015 per request
- ❌ No file access
- ❌ Text responses only
- ⚠️  Only use if you specifically need API mode
...
```bash
cpp "create a test file with hello world"
```
...
**Reading:**
```bash
cpp "show me the contents of config.py" -q
```
**Claude Code:** Uses Read tool, displays content

**Writing:**
```bash
cpp "create a README with project overview"
```
**Claude Code:** Uses Write tool, creates README.md

**Editing:**
```bash
cpp "add error handling to main.py"
```
...
**Adding Features:**
```bash
cpp "add login functionality with JWT authentication"
```
**Claude Code:** Creates files, implements auth, writes tests

**Refactoring:**
```bash
cpp "refactor database.py to use connection pooling"
```
**Claude Code:** Reads code, refactors, tests changes

**Bug Fixing:**
```bash
cpp "fix the memory leak in data_processor.py"
```
...
**Analysis:**
```bash
cpp "analyze sales_data.csv and create summary report"
```
**Claude Code:** Reads CSV, analyzes, creates report

**Visualization:**
```bash
cpp "create bar charts from quarterly_results.json"
```
...
**README Generation:**
```bash
cpp "create comprehensive README with all API endpoints"
```
**Claude Code:** Scans code, documents APIs, creates README

**Comments:**
```bash
cpp "add docstrings to all functions in utils.py"
```
...
**Test Creation:**
```bash
cpp "create unit tests for payment_processor.py"
```
**Claude Code:** Analyzes code, writes test cases

**Running Tests:**
```bash
cpp "run all tests and fix any failures"
```
...
cpp "generate visualization showing the complete methodology" -q
```
...
GUARDRAILS:
✓ Layers 1-3 (Input): Validated
✓ Layers 4-7 (Output): Will validate
...
Execution:
1. ✓ Read methodology.tex
2. ✓ Identified 5 methodology stages
3. ✓ Created generate_methodology_viz.py
4. ✓ Generated methodology_flowchart.png (800x600px)
5. ✓ Verified image quality

Guardrails Validation:
✓ All 7 layers passed

Verification:
✓ Logical: Flowchart matches paper structure
✓ Factual: All stages accurately represented
✓ Complete: All methodology steps included
...
```bash
cpp "fix the bug in payment processing"
```
...
| Feature | Direct Claude Code | cpp (ULTRATHINK) |
|---------|-------------------|--------------------------|
| **File Access** | ✅ Full | ✅ Full |
| **Cost** | $200/month | $200/month (same) |
| **Autonomous Execution** | ⚠️  May ask | ✅ Always |
| **Guardrails** | ❌ No framework | ✅ 7 layers |
| **Verification** | ❌ Informal | ✅ 4 methods |
| **Confidence Scoring** | ❌ No | ✅ Yes (99%+) |
| **Iterative Refinement** | ❌ Manual | ✅ Systematic |
| **Context Management** | ❌ Not tracked | ✅ Full metrics |
| **Quality Guarantee** | ⚠️  Variable | ✅ Production-ready |
| **Validation Framework** | ❌ No | ✅ Comprehensive |
| **Comparison Reporting** | ❌ No | ✅ Yes |
...
### Q: Is cpp the same as typing to Claude Code?
...
**A:** NO - just replace direct prompts with `cpp "prompt"`

### Q: What if I want minimal output?
**A:** Use quiet mode: `cpp "prompt" -q`
...
cpp "your request" --api

# From file
cpp --file prompt.txt

# For web UI
cpp "your request" --web
```
...
1. ✅ **Uses Claude Code by default** (not API)
2. ✅ **No API charges** (included in subscription)
3. ✅ **Same as typing to Claude Code** (enhanced)
4. ✅ **Full ULTRATHINK framework** (all features)
5. ✅ **No breaking changes** (backward compatible)
6. ✅ **Production-ready** (fully tested)
7. ✅ **Systematic validation** (7 guardrails + 4 verification methods)
8. ✅ **Quality guarantee** (99%+ confidence target)
...
```
cpp "prompt" = Typing to Claude Code + ULTRATHINK Framework
```

**You get:**
- ✅ All Claude Code capabilities (file access, execution, everything)
- ✅ No API charges (uses included subscription)
- ✅ ULTRATHINK validation (guardrails, verification, confidence)
- ✅ Production-ready quality (99%+ confidence)
- ✅ Systematic approach (validated at every step)
...
**Use it for ALL your development tasks!** 🚀
