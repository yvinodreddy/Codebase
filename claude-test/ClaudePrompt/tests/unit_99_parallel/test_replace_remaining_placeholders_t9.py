...
def test_all_functions_are_callable():
    ...
    members = inspect.getmembers(replace_remaining_placeholders)
    functions = [name for name, obj in members if inspect.isfunction(obj) and not name.startswith('_')]

    for func_name in functions:
        func = getattr(replace_remaining_placeholders, func_name)
...
def test_all_classes_are_defined():
    ...
    for class_name in classes:
        cls = getattr(replace_remaining_placeholders, class_name)
...
def test_module_docstring_exists():
    ...
    assert replace_remaining_placeholders.__doc__ is not None or replace_remaining_placeholders.__file__ is not None
