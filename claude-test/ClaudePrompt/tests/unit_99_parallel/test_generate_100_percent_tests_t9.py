...
def test_module_has_expected_attributes():
    ...
    public_attrs = [attr for attr in dir(generate_100_percent_tests) if not attr.startswith('_')]
...
def test_all_functions_are_callable():
    ...
    members = inspect.getmembers(generate_100_percent_tests)
    functions = [name for name, obj in members if inspect.isfunction(obj) and not name.startswith('_')]

    for func_name in functions:
        func = getattr(generate_100_percent_tests, func_name)
...
def test_all_classes_are_defined():
    ...
    for class_name in classes:
        cls = getattr(generate_100_percent_tests, class_name)
...
def test_module_docstring_exists():
    ...
    assert generate_100_percent_tests.__doc__ is not None or generate_100_percent_tests.__file__ is not None
