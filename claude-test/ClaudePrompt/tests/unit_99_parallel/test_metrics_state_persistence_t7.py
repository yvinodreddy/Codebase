...
def test_module_has_expected_attributes():
    ...
    assert hasattr(metrics_state_persistence, '__name__')
...
def test_all_functions_are_callable():
    ...
    members = inspect.getmembers(metrics_state_persistence)
    functions = [name for name, obj in members if inspect.isfunction(obj) and not name.startswith('_')]

    for func_name in functions:
        func = getattr(metrics_state_persistence, func_name)
...
def test_all_classes_are_defined():
    ...
    for class_name in classes:
        cls = getattr(metrics_state_persistence, class_name)
...
def test_module_docstring_exists():
    ...
    assert metrics_state_persistence.__doc__ is not None or metrics_state_persistence.__file__ is not None
