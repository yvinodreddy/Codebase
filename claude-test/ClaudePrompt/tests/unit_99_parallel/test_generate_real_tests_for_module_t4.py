...
def test_module_has_expected_attributes():
    ...
    public_attrs = [attr for attr in dir(generate_real_tests_for_module) if not attr.startswith('_')]
...
def test_all_functions_are_callable():
    ...
    members = inspect.getmembers(generate_real_tests_for_module)
    functions = [name for name, obj in members if inspect.isfunction(obj) and not name.startswith('_')]

    for func_name in functions:
        func = getattr(generate_real_tests_for_module, func_name)
...
def test_all_classes_are_defined():
    ...
    for class_name in classes:
        cls = getattr(generate_real_tests_for_module, class_name)
...
def test_module_docstring_exists():
    ...
    assert generate_real_tests_for_module.__doc__ is not None or generate_real_tests_for_module.__file__ is not None
