...
def test_module_has_expected_attributes():
    ...
    assert hasattr(component_introspector, '__name__')

    # Get all public attributes (not starting with _)
    public_attrs = [attr for attr in dir(component_introspector) if not attr.startswith('_')]
...
def test_all_classes_are_defined():
    ...
    for class_name in classes:
        cls = getattr(component_introspector, class_name)
...
def test_module_docstring_exists():
    ...
    assert component_introspector.__doc__ is not None or component_introspector.__file__ is not None
