...
def test_module_has_expected_attributes():
    ...
    assert hasattr(component_introspector_enhanced, '__name__')

    # Get all public attributes (not starting with _)
    public_attrs = [attr for attr in dir(component_introspector_enhanced) if not attr.startswith('_')]
...
def test_all_functions_are_callable():
    ...
    members = inspect.getmembers(component_introspector_enhanced)
    functions = [name for name, obj in members if inspect.isfunction(obj) and not name.startswith('_')]

    for func_name in functions:
        func = getattr(component_introspector_enhanced, func_name)
...
def test_all_classes_are_defined():
    ...
    for class_name in classes:
        cls = getattr(component_introspector_enhanced, class_name)
...
def test_module_docstring_exists():
    ...
    assert component_introspector_enhanced.__doc__ is not None or component_introspector_enhanced.__file__ is not None
