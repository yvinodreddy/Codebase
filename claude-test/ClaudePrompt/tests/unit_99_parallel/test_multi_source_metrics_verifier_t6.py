...
def test_all_functions_are_callable():
    ...
    members = inspect.getmembers(multi_source_metrics_verifier)
    functions = [name for name, obj in members if inspect.isfunction(obj) and not name.startswith('_')]

    for func_name in functions:
        func = getattr(multi_source_metrics_verifier, func_name)
...
def test_all_classes_are_defined():
    ...
    for class_name in classes:
        cls = getattr(multi_source_metrics_verifier, class_name)
...
def test_module_docstring_exists():
    ...
    assert multi_source_metrics_verifier.__doc__ is not None or multi_source_metrics_verifier.__file__ is not None
