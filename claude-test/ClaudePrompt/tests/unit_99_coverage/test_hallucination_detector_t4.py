...
try:
    ...
    spec = importlib.util.spec_from_file_location("hallucination_detector", "guardrails/hallucination_detector.py")
    if spec and spec.loader:
        hallucination_detector = importlib.util.module_from_spec(spec)
        spec.loader.exec_module(hallucination_detector)
...
def test_module_has_attributes():
    ...
    assert len(dir(hallucination_detector)) > 0, "Module should have attributes"
...
def test_module_file_exists():
    ...
    source_path = Path(__file__).parent.parent.parent / "guardrails/hallucination_detector.py"
