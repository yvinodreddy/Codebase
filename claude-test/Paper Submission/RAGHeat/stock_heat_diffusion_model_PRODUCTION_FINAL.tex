...
\text{heat}_{\mathcal{T}}^{(\tau)} = {\small \sum_{i=1}^{K=10}} w_i(\tau) \cdot \phi_i(\tau) + \alpha \cdot \psi(\tau)
\end{equation}
where:
\begin{itemize}
...
{\small \boxed{{\small \sum_{i=1}^{K=10}} w_i(\tau) = 1.0 \quad \forall \tau \geq 0}}
\end{equation}
