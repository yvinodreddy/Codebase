...
\usepackage{breqn}     % Automatic equation breaking  % Better typography
...
\usepackage{breakurl}   % For URL line breaking
\usepackage{microtype}
\usepackage{breqn}     % Automatic equation breaking  % Better typography and spacing
\usepackage{ragged2e}   % Better text justification

{colback=blue!5!white,colframe=blue!75!black,fonttitle=\bfseries,
breakable,enhanced jigsaw}{thm}

{colback=green!5!white,colframe=green!75!black,fonttitle=\bfseries,
breakable,enhanced jigsaw}{lem}

{colback=yellow!5!white,colframe=orange!75!black,fonttitle=\bfseries,
breakable,enhanced jigsaw}{def}

{colback=red!5!white,colframe=red!75!black,fonttitle=\bfseries,
breakable,enhanced jigsaw}{prob}

{colback=cyan!5!white,colframe=cyan!75!black,fonttitle=\bfseries,
breakable,enhanced jigsaw}{cor}
...
\begin{definition}\RaggedRight[Financial Knowledge Graph]
...
\begin{definition}\RaggedRight[Heat Distribution]
...
\begin{definition}\RaggedRight[Dynamic Factor Weights]
A weight allocation is a time-dependent vector $\mathbf{w}(\tau) = [w_1(\tau), \ldots, w_K(\tau)]^T${\small  satisfying:
...
\begin{theorem}\RaggedRight[NP-Hardness of Optimal Weight Selection]
...
We reduce from Partition: Given integers $\{a_1, \ldots, a_n\}$, decide if there exists $S \subseteq \{1, \ldots, n\}$ such that ${\small \sum_{i \in S} a_i = \sum_{i \notin S} a_i}$.
...
The degree matrix $D$ is diagonal with ${\\small D_{ii} = \\sum_{j=1}^{|V|} A_{ij}}$. The normalized symmetric graph Laplacian:
\begin{equation}\small
...
Heat distribution evolves according to the continuous-time heat equation using our novel notation $h^{(\tau)}_i$ to represent heat at node $i$ at continuous time $\tau${\small :
\begin{equation}\small
\label{eq:heat_pde_novel}
{\small \frac{\partial h^{(\tau)}_i}{\partial \tau} = -\beta \sum_{j \in V} \mathcal{L}_{ij} h^{(\tau)}_j}
\end{equation}
...
\begin{theorem}\RaggedRight[Monotonic Convergence under $L_1$ Projection]
...
\begin{proof}
The normalized Laplacian $\mathcal{L}$ is positive semi-definite with eigenvalues $0 = \lambda_1 \leq \lambda_2 \leq \cdots \leq \lambda_{|V|} \leq 2${\small . The heat kernel admits spectral decomposition:
\begin{equation}\small
{\small \exp(-\beta \mathcal{L} \tau) = \sum_{k=1}^{|V|} e^{-\beta \lambda_k \tau} \mathbf{u}_k \mathbf{u}_k^T}
\end{equation}
where }$\{\mathbf{u}_k\}$ are orthonormal eigenvectors.

Since $\lambda_1 = 0$ with $\mathbf{u}_1 = \mathbf{1}/\sqrt{|V|}${\small  (constant vector), the steady-state is:
\begin{equation}\small
...
For }$\epsilon$-convergence: $e^{-\beta \lambda_2 \tau} \leq \epsilon$ requires $\tau \geq \frac{1}{\beta \lambda_2} \log(1/\epsilon)${\small .
\end{proof}

\begin{theorem}\RaggedRight[Approximation Bound via Truncated Heat Kernel]
...
\end{equation}
For }$\epsilon$-accuracy, $M = O(\log(1/\epsilon))${\small  terms suffice.
\end{theorem}
...
\begin{lemma}\RaggedRight[Bounded Weight Drift Guarantee]
...
\begin{lemma}\RaggedRight[Regime-Adaptive Stability Condition]
\label{lem:stability}
...
\begin{lemma}\RaggedRight[Monotonic Convergence under Projection]
\label{lem:monotonic}
...
\begin{theorem}\RaggedRight[Computational Complexity]
...
For target stock $\mathcal{T}${\small , aggregated heat score combines diffused heat from all factor categories with dynamic weights:
\begin{equation}\small
...
\item $\psi(\tau)${\small  captures multi-hop influences beyond direct factors
\end{itemize}
...
\end{split}
\end{equation}
where }$\mathcal{N}(\mathcal{T})$ is neighborhood, $\text{att}(j, \mathcal{T})$ is attention weight, $\rho(\mathcal{T}, j)${\small  is historical return correlation.
...
\item \textbf{Directional Accuracy:} ${\small \text{Acc} = \frac{1}{N} \sum_{i=1}^{N} \mathbb{I}[\text{sign}(\hat{r}_i) = \text{sign}(r_i)]}$
