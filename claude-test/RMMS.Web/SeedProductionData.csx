...
#r "./RMMS.DataAccess/bin/Debug/net8.0/RMMS.DataAccess.dll"
#r "./RMMS.Models/bin/Debug/net8.0/RMMS.Models.dll"
...
// Helper function to generate random date in last N days
DateTime RandomDate(int daysAgo)
{
    var days = random.Next(1, daysAgo);
    var hours = random.Next(0, 24);
    return DateTime.Now.AddDays(-days).Date.AddHours(hours);
}

// Helper function to add some variance
decimal Variance(decimal value, double percentRange)
{
    var variance = value * (decimal)(random.NextDouble() * percentRange * 2 - percentRange);
...
try
{
    ...
    var operators = new[] { "John Doe", "Jane Smith", "Mike Johnson", "Sarah Williams", "David Brown" };
    ...
    for (int i = 0; i < 30; i++)
    {
        ...
        var inputQuantity = Variance(order.PaddyQuantity, 0.02); // +/- 2% variance
        ...
        var batch = new ProductionBatch
        {
            ...
            QualityScore = Variance(8.0M, 0.15), // Quality score 6.8-9.2
            IsActive = true,
        ...
        var batchInput = new BatchInput
        {
            ...
            MoistureContent = Variance(14.0M, 0.1),
            Grade = "Grade A",
        ...
        // Base yields with some randomness
        var headRicePercent = Variance(65.0M, 0.08); // 60-70% typical
        var brokenRicePercent = Variance(6.0M, 0.3); // 4-8%
        var branPercent = Variance(8.0M, 0.15); // 7-9%
        var huskPercent = Variance(20.0M, 0.1); // 18-22%
        var wastagePercent = Variance(1.0M, 0.5); // 0.5-1.5%
