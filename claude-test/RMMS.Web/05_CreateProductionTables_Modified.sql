-- =====================================================
-- CREATE PRODUCTION MANAGEMENT TABLES (MODIFIED)
-- =====================================================
-- Sprint 3: Production & Milling Operations
-- MODIFIED VERSION: Removed FK constraints to missing tables
-- Tables removed: Products, Employees, StorageZones
-- These FK constraints can be added later when tables exist
-- =====================================================
...
PRINT 'MODIFIED VERSION - Without missing table dependencies';
PRINT '======================================================';
PRINT '';
...
(
    ...
    -- Paddy Information (FK to Products removed - will be added when Products table exists)
    PaddyProductId INT NULL,  -- Changed from NOT NULL to allow creation without FK
    PaddyVariety NVARCHAR(50) NULL,
    PaddyQuantity DECIMAL(18,3) NOT NULL,
    PaddyUOM NVARCHAR(10) NOT NULL DEFAULT 'Quintals',

    -- Target Rice Information (FK to Products removed)
    ...
    AssignedSupervisorId INT NULL,  -- FK to Employees removed - will be added later
    ...
    -- Foreign Keys (only to tables that exist)
    CONSTRAINT FK_ProductionOrders_Machine FOREIGN KEY (AssignedMachineId)
        REFERENCES dbo.Machines(Id),

    -- NOTE: The following FKs are commented out until tables exist:
    -- CONSTRAINT FK_ProductionOrders_PaddyProduct FOREIGN KEY (PaddyProductId) REFERENCES dbo.Products(Id),
    -- CONSTRAINT FK_ProductionOrders_TargetRiceProduct FOREIGN KEY (TargetRiceProductId) REFERENCES dbo.Products(Id),
    -- CONSTRAINT FK_ProductionOrders_Supervisor FOREIGN KEY (AssignedSupervisorId) REFERENCES dbo.Employees(Id),
...
(
    ...
    -- Personnel (FKs to Employees removed - will be added later)
    OperatorId INT NULL,
    ...
    -- NOTE: The following FKs are commented out until Employees table exists:
    -- CONSTRAINT FK_ProductionBatches_Operator FOREIGN KEY (OperatorId) REFERENCES dbo.Employees(Id),
    -- CONSTRAINT FK_ProductionBatches_Supervisor FOREIGN KEY (SupervisorId) REFERENCES dbo.Employees(Id),
...
(
    ...
    -- Input Information (FK to Products removed)
    InputType NVARCHAR(50) NOT NULL DEFAULT 'Paddy',
    ProductId INT NULL,  -- Changed from NOT NULL
    Quantity DECIMAL(18,3) NOT NULL,
    UOM NVARCHAR(10) NOT NULL DEFAULT 'Quintals',

    -- Source Location
    WarehouseId INT NULL,  -- Changed from NOT NULL
    ZoneId INT NULL,
    ...
    -- NOTE: The following FKs are commented out until tables exist:
    -- CONSTRAINT FK_BatchInputs_Product FOREIGN KEY (ProductId) REFERENCES dbo.Products(Id),
    -- CONSTRAINT FK_BatchInputs_Zone FOREIGN KEY (ZoneId) REFERENCES dbo.StorageZones(Id) ON DELETE SET NULL,
...
(
    ...
    -- Output Information (FK to Products removed)
    ...
    -- NOTE: The following FKs are commented out until tables exist:
    -- CONSTRAINT FK_BatchOutputs_Product FOREIGN KEY (ProductId) REFERENCES dbo.Products(Id),
    -- CONSTRAINT FK_BatchOutputs_Zone FOREIGN KEY (ZoneId) REFERENCES dbo.StorageZones(Id) ON DELETE SET NULL,
...
PRINT 'Foreign Keys: 8 (Reduced - missing table FKs commented out)';
PRINT 'Unique Indexes: 3';
PRINT 'Performance Indexes: 24';
PRINT '';
PRINT 'NOTE: Some Foreign Key constraints are commented out:';
PRINT '  - FK to Products table (PaddyProductId, TargetRiceProductId, etc.)';
PRINT '  - FK to Employees table (OperatorId, SupervisorId, etc.)';
PRINT '  - FK to StorageZones table (ZoneId)';
PRINT 'These will need to be added when those tables are created.';
PRINT '';
PRINT 'Next Steps:';
