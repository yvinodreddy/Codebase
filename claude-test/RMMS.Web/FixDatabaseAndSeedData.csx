...
Console.WriteLine("===========================================");
Console.WriteLine("RMMS Database Fix and Seed Data Script");
Console.WriteLine("===========================================\n");

using (var connection = new SqlClient(connectionString))
{
    connection.Open();
    Console.WriteLine("✓ Connected to database successfully\n");

    // Fix InventoryLedger TotalValue column
    Console.WriteLine("1. Fixing InventoryLedger schema...");
    ExecuteNonQuery(connection, @"
        ...
        BEGIN
            ...
            PRINT '   Added TotalValue column';
        END
    ");
    Console.WriteLine("✓ InventoryLedger schema fixed\n");

    //  Seed Master Data
    Console.WriteLine("2. Seeding Master Data...");
    ...
    // Seed Inventory Data
    Console.WriteLine("3. Seeding Inventory Data...");
    ...
    // Seed Inventory Ledger (initial stock for all products in main warehouse)
    ExecuteNonQuery(connection, @"
        IF NOT EXISTS (SELECT 1 FROM InventoryLedger WHERE ProductId = 1 AND WarehouseId = 1)
        BEGIN
            ...
            BEGIN
                ...
                SELECT
                    ...
                    @WarehouseId,
                    CASE
                        WHEN @ProductId <= 4 THEN 200.000  -- Finished Rice
                        WHEN @ProductId <= 6 THEN 500.000  -- Paddy
                        ELSE 100.000                        -- By-products
                    END AS CurrentStock,
                    CASE
                        WHEN @ProductId <= 4 THEN 50.000
                        WHEN @ProductId <= 6 THEN 200.000
                        ELSE 30.000
                    END AS MinimumLevel,
                    CASE
                        WHEN @ProductId <= 4 THEN 500.000
                        WHEN @ProductId <= 6 THEN 2000.000
                        ELSE 300.000
                    END AS MaximumLevel,
                    CASE
                        WHEN @ProductId <= 4 THEN 100.000
                        WHEN @ProductId <= 6 THEN 400.000
                        ELSE 50.000
                    END AS ReorderLevel,
                    PurchaseRate AS UnitCost,
                    CASE
                        WHEN @ProductId <= 4 THEN 200.000 * PurchaseRate
                        WHEN @ProductId <= 6 THEN 500.000 * PurchaseRate
                        ELSE 100.000 * PurchaseRate
                    END AS TotalValue,
                    GETDATE(),
                    1,
                    GETDATE(),
            ...
            PRINT '   Seeded Inventory Ledger for 15 products';
        END
    ");
    ...
    Console.WriteLine("4. Seeding Production Data...");
    ...
    Console.WriteLine("✓ Production Data seeded successfully\n");
    ...
    Console.WriteLine("  - 10 Vendors");
    Console.WriteLine("  - 15 Products");
    Console.WriteLine("  - 10 Employees");
    Console.WriteLine("  - 5 Warehouses");
    Console.WriteLine("  - 15 Inventory Records");
    Console.WriteLine("  - 5 Machines");
    Console.WriteLine("  Total: 70 records\n");
}
