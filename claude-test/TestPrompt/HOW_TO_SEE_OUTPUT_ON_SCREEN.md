...
```bash
ultrathinkc "your prompt here" -v > /tmp/ultrathink_output.txt
```

**What this does**:
- Runs your command
- Saves output to `/tmp/ultrathink_output.txt`
...
```bash
ultrathinkc "your prompt" -v 2>&1 | tee /tmp/output.txt
```
...
When you send me an `ultrathinkc` command, I do this:

```bash
# 1. Run and save
ultrathinkc "your prompt" -v 2>&1 > /tmp/ultrathink_output.txt
...
ultrathinkc --file /tmp/my_tasks.txt -v > /tmp/results.txt
...
ultrathinkc --file /tmp/large_prompt.txt -v > /tmp/output.txt
cat /tmp/output.txt
```
...
### For Normal Prompts
```bash
ultrathinkc "prompt" -v > /tmp/out.txt && cat /tmp/out.txt
```
...
# Run and display
ultrathinkc --file /tmp/prompt.txt -v > /tmp/out.txt && cat /tmp/out.txt
```
...
```bash
ultrathinkc "prompt" -v > ~/my_results/output_$(date +%Y%m%d_%H%M%S).txt
```
...
```bash
ultrathinkc "prompt" -v > results.txt
...
ultrathinkc --file /tmp/project_tasks.txt -v > /tmp/project_results.txt
...
```bash
# Iteration 1
ultrathinkc "prompt v1" -v > /tmp/iter1.txt
cat /tmp/iter1.txt

# Review, refine, iterate
ultrathinkc "prompt v2 (refined)" -v > /tmp/iter2.txt
cat /tmp/iter2.txt
...
1. Run: `ultrathinkc "prompt" -v > /tmp/out.txt`
