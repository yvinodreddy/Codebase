...
You now have complete documentation and testing tools to learn ultrathinkc:
...
3. **test_ultrathinkc.sh** - Basic verification
...
**Results saved to:** `/tmp/ultrathinkc_tests_[timestamp]/`
...
**Results saved to:** `/tmp/ultrathinkc_comparison_[timestamp]/`
...
```bash
# Math
./ultrathinkc "Calculate 15% of 250"

# Explanation
./ultrathinkc "Explain how DNS works"

# Comparison
./ultrathinkc "Compare REST vs GraphQL"
```
...
```bash
# Function
./ultrathinkc "Write a Python function to validate email addresses"

# Class
./ultrathinkc "Create a Python class for a binary search tree"

# Algorithm
./ultrathinkc "Implement merge sort in Python with comments"
```
...
```bash
# Architecture
./ultrathinkc "Design a microservices architecture for e-commerce"

# API
./ultrathinkc "Design a RESTful API for user management"

# Database
./ultrathinkc "Design a database schema for a blogging platform"
```
...
# Process it
./ultrathinkc --file /tmp/complex_task.txt --verbose
```
...
./ultrathinkc "Explain type 2 diabetes treatment options" --verbose
...
- [ ] Can explain what ultrathinkc does
...
- [ ] Know when to use vs not use ultrathinkc
...
### Q: How much does ultrathinkc cost in tokens?
...
```bash
# Example CI/CD usage
./ultrathinkc "Review code changes" --api > review.txt
```
...
└── test_ultrathinkc.sh                      ← Basic test
```
