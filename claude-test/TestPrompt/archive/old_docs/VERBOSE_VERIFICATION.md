...
```bash
ultrathinkc "what is 2+2" --verbose 2>/dev/null | grep -E "FRAMEWORK|STAGE|Layer|Agent Component|Iteration"
```

### Test 2: Count sections
```bash
# Should show exactly 6 stages
ultrathinkc "test" --verbose 2>/dev/null | grep "STAGE [0-9]:" | wc -l

# Should show exactly 7 layers
ultrathinkc "test" --verbose 2>/dev/null | grep "Layer [0-9]:" | wc -l

# Should show exactly 3 agent components
ultrathinkc "test" --verbose 2>/dev/null | grep "Agent Component:" | wc -l

# Should show framework benefits section
ultrathinkc "test" --verbose 2>/dev/null | grep "FRAMEWORK - WHAT YOU'RE GETTING"

# Should show iteration details
ultrathinkc "test" --verbose 2>/dev/null | grep "Iteration [0-9]/[0-9]"

# Should show framework comparison
ultrathinkc "test" --verbose 2>/dev/null | grep "FRAMEWORK COMPARISON"
```

### Test 3: Full output verification
```bash
# Save full output to file
ultrathinkc "what is 2+2" --verbose > /tmp/ultrathink_test.txt 2>&1
...
The `ultrathinkc` command works from any directory:
- Installed at: `/home/user01/.local/bin/ultrathinkc`
- Points to: `/home/user01/claude-test/TestPrompt/ultrathinkc`
- Calls: `/home/user01/claude-test/TestPrompt/ultrathink.py`
