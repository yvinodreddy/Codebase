...
ultrathinkc "generate new images based on latex paper" -q
```
...
**Old Code (TestPrompt_old) - Line 417-421:**
```python
parser.add_argument(
    '--api',
...
**New Code (TestPrompt) - Lines 1011-1017 (BEFORE FIX):**
```python
parser.add_argument(
    '--no-api',
    dest='api',
...
ultrathinkc "list all python files" -q 2>/dev/null
```
...
```
## Python Files in /home/user01/claude-test/TestPrompt
...
```bash
cd "/home/user01/claude-test/Paper Submission/RAGHeat"
ultrathinkc "generate new images based on latex paper can you work on it and let me know results" -q
```
...
ultrathinkc "add new equations to section 3" -q
```
...
ultrathinkc "add error handling to all API calls"
```
...
ultrathinkc "create visualizations for the latest results" -q
```
...
```bash
cd "/path/to/docs"
ultrathinkc "update README with new features"
```
...
```bash
ultrathinkc "explain this code" --api
```
...
```bash
ultrathinkc "test prompt" --api --verbose
```
...
```bash
ultrathinkc "prompt"        # Used API (broken)
ultrathinkc "prompt" --no-api  # Generated prompt (workaround)
```

**After (fixed version):**
```bash
ultrathinkc "prompt"        # Generates prompt (correct!)
ultrathinkc "prompt" --api  # Uses API (if you want)
```
...
6. ✅ **Backwards compatible** - Matches old TestPrompt_old behavior
...
1. **Add `--mode` flag** - Explicit mode selection:
   ```bash
   ultrathinkc "prompt" --mode=code    # Default
   ultrathinkc "prompt" --mode=api     # API mode
   ultrathinkc "prompt" --mode=web     # Web format
   ```
...
- ✅ **Use ultrathinkc for its intended purpose**
