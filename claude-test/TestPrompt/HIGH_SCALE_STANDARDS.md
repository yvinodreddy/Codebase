...
ultrathinkc "large prompt" -v > /tmp/ultrathink_output.txt
...
ultrathinkc "prompt" -v 2>&1 | tee /tmp/output.txt
...
1. **Use file-based input**:
   ```bash
   ultrathinkc --file /path/to/tasks.txt -v > /tmp/output.txt
   ```
