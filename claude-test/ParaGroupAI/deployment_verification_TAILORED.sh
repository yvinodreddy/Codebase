#!/usr/bin/env bash
#
# ClaudePrompt/ULTRATHINK Framework - TAILORED Deployment Verification
# CUSTOMIZED FOR: semanticraj@Ubuntu 22.04.5 LTS (QEMU/UTM VM)
#
# This script performs comprehensive verification tailored to your system
#
# Usage: bash deployment_verification_TAILORED.sh
#
# Exit codes:
...
# TAILORED paths for semanticraj
BASE_DIR="/home/semanticraj/claude-test"
CLAUDE_PROMPT_DIR="$BASE_DIR/ClaudePrompt"
TEST_PROMPT_DIR="$BASE_DIR/TestPrompt"
...
test_directory_structure() {
    ...
    local REQUIRED_DIRS=(
        "$BASE_DIR"
        "$CLAUDE_PROMPT_DIR"
        "$CLAUDE_PROMPT_DIR/tmp"
        "$CLAUDE_PROMPT_DIR/agent_framework"
        "$CLAUDE_PROMPT_DIR/guardrails"
        "$CLAUDE_PROMPT_DIR/security"
        "$TEST_PROMPT_DIR"
    )
...
test_required_files() {
    ...
    local REQUIRED_FILES=(
        "$BASE_DIR/CLAUDE.md"
        "$CLAUDE_PROMPT_DIR/CLAUDE.md"
        "$CLAUDE_PROMPT_DIR/cpp"
        "$CLAUDE_PROMPT_DIR/cpps"
        "$CLAUDE_PROMPT_DIR/ultrathink.py"
        "$CLAUDE_PROMPT_DIR/master_orchestrator.py"
        "$CLAUDE_PROMPT_DIR/config.py"
        "$CLAUDE_PROMPT_DIR/get_output_path.py"
        "$CLAUDE_PROMPT_DIR/answer_to_file.py"
        "$CLAUDE_PROMPT_DIR/requirements.txt"
        "$TEST_PROMPT_DIR/ultrathink.py"
        "$TEST_PROMPT_DIR/answer_to_file.py"
    )
...
test_bashrc_aliases() {
    print_header "TEST 4: Bashrc Aliases Verification (TAILORED)"
    ...
    # Check aliases - NOTE: 'cpp' is now 'ccode' to avoid conflict
    if alias ccode &>/dev/null; then
        log_success "Alias 'ccode' is defined (replaces 'cpp' to avoid conflict)"
    else
        log_failure "Alias 'ccode' is NOT defined"
    fi

    if alias cpps &>/dev/null; then
        log_success "Alias 'cpps' is defined"
    else
        log_failure "Alias 'cpps' is NOT defined"
    fi
    ...
    # Verify no cpp conflict
    if [ -f "/usr/bin/cpp" ]; then
        log_success "C preprocessor at /usr/bin/cpp - correctly avoided by using 'ccode' alias"
    fi
}
...
test_python_installation() {
    ...
    if command -v python3 &>/dev/null; then
        ...
        if [[ "$PY_VERSION" == "3.10."* ]]; then
            log_success "Python3 3.10.x is installed: $PY_VERSION"
        else
            log_warning "Python3 version is $PY_VERSION (expected 3.10.x)"
            ((TESTS_PASSED++))
            ((TOTAL_TESTS++))
        fi
    else
...
test_python_dependencies() {
    ...
    for pkg in "${REQUIRED_PACKAGES[@]}"; do
        if pip3 list 2>/dev/null | grep -i "^$pkg " &>/dev/null; then
            ...
            log_warning "Package not installed: $pkg - Run: pip3 install --user $pkg"
            ((TESTS_PASSED++))
            ((TOTAL_TESTS++))
        fi
    done
...
test_command_execution() {
    print_header "TEST 7: Command Execution Test (TAILORED)"

    # Test if cpp wrapper exists and is executable
    if [ -x "$CLAUDE_PROMPT_DIR/cpp" ]; then
        log_success "cpp wrapper is executable"

        # Test with --help flag
        if $CLAUDE_PROMPT_DIR/cpp --help &>/dev/null; then
    ...
    # Test cpps wrapper
    if [ -x "$CLAUDE_PROMPT_DIR/cpps" ]; then
        log_success "cpps wrapper is executable"
    else
        log_failure "cpps wrapper is NOT executable"
    fi
}
...
test_output_directory() {
    ...
    local TMP_DIR="$CLAUDE_PROMPT_DIR/tmp"
...
test_get_output_path() {
    ...
    local SCRIPT="$CLAUDE_PROMPT_DIR/get_output_path.py"
...
test_answer_to_file() {
    ...
    local SCRIPT="$CLAUDE_PROMPT_DIR/answer_to_file.py"
...
test_claude_md_files() {
    ...
    local CLAUDE_MD_ROOT="$BASE_DIR/CLAUDE.md"
    local CLAUDE_MD_CP="$CLAUDE_PROMPT_DIR/CLAUDE.md"
...
test_isolation() {
    ...
    local TP_ANSWER="$TEST_PROMPT_DIR/answer_to_file.py"
    local CP_ANSWER="$CLAUDE_PROMPT_DIR/answer_to_file.py"
...
test_user_verification() {
    print_header "TEST 13: User and Path Verification (TAILORED)"

    # Verify running as semanticraj
    if [ "$USER" = "semanticraj" ]; then
        log_success "Running as expected user: semanticraj"
    else
        log_warning "Running as $USER (expected semanticraj)"
        ((TESTS_PASSED++))
        ((TOTAL_TESTS++))
    fi

    # Verify home directory
    if [ "$HOME" = "/home/semanticraj" ]; then
        log_success "Home directory is correct: /home/semanticraj"
    else
        log_warning "Home directory is $HOME (expected /home/semanticraj)"
        ((TESTS_PASSED++))
        ((TOTAL_TESTS++))
    fi

    # Verify QEMU virtualization
    if command -v systemd-detect-virt &>/dev/null; then
        VIRT_TYPE=$(systemd-detect-virt)
        if [ "$VIRT_TYPE" = "qemu" ]; then
            log_success "Running on QEMU/UTM as expected"
        else
            log_warning "Virtualization type: $VIRT_TYPE (expected qemu)"
            ((TESTS_PASSED++))
            ((TOTAL_TESTS++))
        fi
    fi
...
test_claude_code() {
    print_header "TEST 14: Claude Code Installation"

    if command -v claude &>/dev/null; then
        log_success "Claude Code is installed"

        VERSION=$(claude --version 2>&1 | head -n 1)
        log_success "Claude Code version: $VERSION"
    else
        log_failure "Claude Code is NOT installed"
    fi
}
...
main() {
    print_header "ClaudePrompt/ULTRATHINK Framework - TAILORED Deployment Verification"

    log_info "Verification for: semanticraj@Ubuntu 22.04.5 LTS (QEMU/UTM)"
    ...
    if [ $TESTS_FAILED -eq 0 ]; then
        ...
        echo "" | tee -a "$LOG_FILE"
        log_info "IMPORTANT: Use 'ccode' instead of 'cpp' (avoid conflict with C preprocessor)"
        return 0
    else
