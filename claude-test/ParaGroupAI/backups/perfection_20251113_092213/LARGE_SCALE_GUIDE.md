...
cpp --file /tmp/large_prompt.txt

# Method 2: Direct (works up to ARG_MAX limit)
cpp "$(cat /tmp/large_prompt.txt)" --verbose
```
...
# Process with cpp
cpp --file /tmp/tasks.txt --verbose > /tmp/task_analysis.txt
...
stream, return_code = stream_cpp_command(
...
success, result, errors = handler.retry_with_backoff(
    operation=lambda: run_cpp_command(),
    operation_name="cpp_call",
    max_retries=5,
    initial_delay=1.0,
    max_delay=60.0
)
