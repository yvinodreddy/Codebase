...
|------|---------|----------|--------|--------|
| Default mode | `prsg "list files" -q` | Works without API | Worked | ✅ PASS |
| API mode no key | `prsg "test" --api` | Shows error | Showed error | ✅ PASS |
...
- `prsg "list python files" -q` (test default mode)
- `prsg "test" --api` (test API mode error)
