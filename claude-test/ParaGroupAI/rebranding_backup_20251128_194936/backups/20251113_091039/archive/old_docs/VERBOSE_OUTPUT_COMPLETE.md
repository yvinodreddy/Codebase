...
### 1. `/home/user01/claude-test/ClaudePrompt/master_orchestrator.py`
...
```bash
$ cpp --file large_prompt_5000chars.txt --verbose
```
...
# From file
cpp --file requirements.txt --verbose
...
```bash
cpp "prompt" --verbose 2>/dev/null | grep -E "\[VERBOSE\]|^==="
```

**Save formatted output to file**:
```bash
cpp "prompt" --verbose 2>/dev/null > output.txt
```
