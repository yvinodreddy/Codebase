"""
Comprehensive test suite for dependency_scanner
Auto-generated for 99%+ coverage achievement
Module: security/dependency_scanner.py
...
def module_path():
    ...
    return project_root / "security/dependency_scanner.py"
...
def module_under_test(module_path):
    ...
    if not module_path.exists():
        pytest.skip(f"Module not found: security/dependency_scanner.py")

    try:
        spec = importlib.util.spec_from_file_location("dependency_scanner", module_path)
        if spec is None or spec.loader is None:
            pytest.skip(f"Cannot create spec for module: security/dependency_scanner.py")

        module = importlib.util.module_from_spec(spec)
        sys.modules["dependency_scanner"] = module
...
class TestModuleClasses:
    ...
    def test_classes_can_be_instantiated(self, module_under_test):
        ...
        public_classes = [(name, cls) for name, cls in classes
                         if not name.startswith('_') and cls.__module__ == "dependency_scanner"]
