"""
Comprehensive test suite for azure_content_safety
Auto-generated for 99%+ coverage achievement
Module: guardrails/azure_content_safety.py
...
def module_path():
    ...
    return project_root / "guardrails/azure_content_safety.py"
...
def module_under_test(module_path):
    ...
    if not module_path.exists():
        pytest.skip(f"Module not found: guardrails/azure_content_safety.py")

    try:
        spec = importlib.util.spec_from_file_location("azure_content_safety", module_path)
        if spec is None or spec.loader is None:
            pytest.skip(f"Cannot create spec for module: guardrails/azure_content_safety.py")

        module = importlib.util.module_from_spec(spec)
        sys.modules["azure_content_safety"] = module
...
class TestModuleClasses:
    ...
    def test_classes_can_be_instantiated(self, module_under_test):
        ...
        public_classes = [(name, cls) for name, cls in classes
                         if not name.startswith('_') and cls.__module__ == "azure_content_safety"]
