# PARA GROUP AI ORCHESTRATOR® - COMPREHENSIVE REBRANDING PLAN
...
This plan provides complete, production-ready rebranding for the ParaGroupAI directory, eliminating all "ClaudePrompt" and "cpp" references in favor of "Para Group AI Orchestrator®" and "prsg" branding.

**Scope**: ParaGroupAI directory + Root CLAUDE.md file
**Timeline**: Immediate execution (< 30 minutes)
**Breaking Changes**: ZERO (backward compatibility via cpp→prsg symlink)
**Validation**: Automated testing after each phase
...
- `/home/user01/claude-test/ParaGroupAI/CLAUDE.md` - 46 cpp/ClaudePrompt references
- `/home/user01/claude-test/CLAUDE.md` - 6 ClaudePrompt path references

**2. Production Scripts** (COMPLETED ✅):
- `/home/user01/claude-test/ParaGroupAI/prsg` - ✅ Already fixed
- `/home/user01/claude-test/ParaGroupAI/cpp_core` - ✅ Already fixed
- `/home/user01/claude-test/ParaGroupAI/capture_context_helper.sh` - ✅ Already fixed

**3. README Files** (PENDING):
- Search for all README.md files needing trademark notices

**4. Archive Directories** (PENDING - Low Priority):
- Fix references in archived code (if needed)
...
```
 ...
 M .ultrathink/current_session.json
 M .ultrathink/project_mappings.json
 M ParaGroupAI/capture_context_helper.sh
 M ParaGroupAI/cpp_core
 M ParaGroupAI/prsg
```
...
### PHASE 1: Fix ParaGroupAI/CLAUDE.md (46 references)

**Replacements Needed:**

1. **Command References** (cpp → prsg):
   - Line 7: "cpp NOW PRESERVES" → "prsg NOW PRESERVES"
   - Line 13: "Run cpp from" → "Run prsg from"
   - Line 16: "cpp \"your question\"" → "prsg \"your question\""
   - Line 31: "ClaudePrompt/tmp" → "ParaGroupAI/tmp"
   - Line 32: "ClaudePrompt/tmp/cppultrathink" → "ParaGroupAI/tmp/prsgultrathink"
   - Line 39: "cpp \"question\"" → "prsg \"question\""
   - Line 51-54: Update modified files references

2. **Path References** (ClaudePrompt → ParaGroupAI):
   - Line 143: "/home/user01/claude-test/ClaudePrompt/database/dual_context_retriever.py" → "ParaGroupAI"
   - Line 247: Same pattern
   - Line 380: cd /home/user01/claude-test/ClaudePrompt → ParaGroupAI
   - Line 475-476: Documentation path references
   - Line 556-558: Documentation path references
   - Line 587: "ClaudePrompt": Command `cpp` → "ParaGroupAI": Command `prsg`
   - Lines 593-811: Multiple references in ULTRATHINK protocol section

3. **Product Name References**:
   - "ClaudePrompt - THIS DIRECTORY" → "ParaGroupAI - THIS DIRECTORY"
   - File name references in outputs
   - Example commands

### PHASE 2: Fix Root CLAUDE.md (6 references)

**Replacements Needed:**

1. Line 135: `/home/user01/claude-test/ClaudePrompt/CLAUDE.md` →
   Use both ClaudePrompt (for reference) AND ParaGroupAI

2. Line 155: "cpp command execution (in ClaudePrompt system)" →
   "prsg command execution (in ParaGroupAI system)"

3. Line 190: `/home/user01/claude-test/ClaudePrompt/ultrathink.py` →
   `/home/user01/claude-test/ParaGroupAI/ultrathink.py`

4. Line 337-398: Update full documentation path references

### PHASE 3: Add Trademark Notices to README Files
...
### PHASE 4: Archive Directory Cleanup (Low Priority)

- Search for archive directories
- Update references if active code depends on them
- Otherwise mark as legacy/deprecated

================================================================================
## EXECUTION SCRIPT
================================================================================

The following script will be created to automate all changes:

**File**: `/home/user01/claude-test/ParaGroupAI/EXECUTE_COMPREHENSIVE_REBRAND.sh`
...
After executing the script, validate:

- [ ] prsg command works: `./prsg --help`
- [ ] cpp symlink works: `./cpp --help` (should call prsg)
- [ ] No "ClaudePrompt" in production files: `grep -r "ClaudePrompt" prsg cpp_core capture_context_helper.sh`
- [ ] CLAUDE.md files updated: Check both ParaGroupAI and root
- [ ] Trademark notices added: Check README files
- [ ] Git diff shows expected changes: `git diff --stat`
- [ ] All tests pass: Run any existing test suites
...
```bash
# Restore from backup
BACKUP_DIR="<backup directory from execution>"
cp "$BACKUP_DIR"/* /home/user01/claude-test/ParaGroupAI/
cp "$BACKUP_DIR/root_CLAUDE.md.backup" /home/user01/claude-test/CLAUDE.md
...
**Expected Results**:
- 0 "ClaudePrompt" references in production code
- 0 standalone "cpp" command references (except symlink)
- 100% trademark compliance
- 100% backward compatibility (via cpp symlink)
