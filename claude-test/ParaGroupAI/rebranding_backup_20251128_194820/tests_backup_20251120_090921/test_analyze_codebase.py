"""
Comprehensive test suite for analyze_codebase
Auto-generated for 99%+ coverage achievement
Module: analyze_codebase.py
...
def module_path():
    ...
    return project_root / "analyze_codebase.py"
...
def module_under_test(module_path):
    ...
    if not module_path.exists():
        pytest.skip(f"Module not found: analyze_codebase.py")

    try:
        spec = importlib.util.spec_from_file_location("analyze_codebase", module_path)
        if spec is None or spec.loader is None:
            pytest.skip(f"Cannot create spec for module: analyze_codebase.py")

        module = importlib.util.module_from_spec(spec)
        sys.modules["analyze_codebase"] = module
...
class TestModuleClasses:
    ...
    def test_classes_can_be_instantiated(self, module_under_test):
        ...
        public_classes = [(name, cls) for name, cls in classes
                         if not name.startswith('_') and cls.__module__ == "analyze_codebase"]
