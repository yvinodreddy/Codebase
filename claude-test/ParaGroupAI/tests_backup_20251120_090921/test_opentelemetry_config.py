"""
Comprehensive test suite for opentelemetry_config
Auto-generated for 99%+ coverage achievement
Module: infrastructure/tracing/opentelemetry_config.py
...
def module_path():
    ...
    return project_root / "infrastructure/tracing/opentelemetry_config.py"
...
def module_under_test(module_path):
    ...
    if not module_path.exists():
        pytest.skip(f"Module not found: infrastructure/tracing/opentelemetry_config.py")

    try:
        spec = importlib.util.spec_from_file_location("opentelemetry_config", module_path)
        if spec is None or spec.loader is None:
            pytest.skip(f"Cannot create spec for module: infrastructure/tracing/opentelemetry_config.py")

        module = importlib.util.module_from_spec(spec)
        sys.modules["opentelemetry_config"] = module
...
class TestModuleClasses:
    ...
    def test_classes_can_be_instantiated(self, module_under_test):
        ...
        public_classes = [(name, cls) for name, cls in classes
                         if not name.startswith('_') and cls.__module__ == "opentelemetry_config"]
