"""
Comprehensive test suite for performance_profiler
Auto-generated for 99%+ coverage achievement
Module: infrastructure/performance_profiler.py
...
def module_path():
    ...
    return project_root / "infrastructure/performance_profiler.py"
...
def module_under_test(module_path):
    ...
    if not module_path.exists():
        pytest.skip(f"Module not found: infrastructure/performance_profiler.py")

    try:
        spec = importlib.util.spec_from_file_location("performance_profiler", module_path)
        if spec is None or spec.loader is None:
            pytest.skip(f"Cannot create spec for module: infrastructure/performance_profiler.py")

        module = importlib.util.module_from_spec(spec)
        sys.modules["performance_profiler"] = module
...
class TestModuleClasses:
    ...
    def test_classes_can_be_instantiated(self, module_under_test):
        ...
        public_classes = [(name, cls) for name, cls in classes
                         if not name.startswith('_') and cls.__module__ == "performance_profiler"]
