"""
Comprehensive test suite for task_archiver
Auto-generated for 99%+ coverage achievement
Module: task_archiver.py
...
def module_path():
    ...
    return project_root / "task_archiver.py"
...
def module_under_test(module_path):
    ...
    if not module_path.exists():
        pytest.skip(f"Module not found: task_archiver.py")

    try:
        spec = importlib.util.spec_from_file_location("task_archiver", module_path)
        if spec is None or spec.loader is None:
            pytest.skip(f"Cannot create spec for module: task_archiver.py")

        module = importlib.util.module_from_spec(spec)
        sys.modules["task_archiver"] = module
...
class TestModuleClasses:
    ...
    def test_classes_can_be_instantiated(self, module_under_test):
        ...
        public_classes = [(name, cls) for name, cls in classes
                         if not name.startswith('_') and cls.__module__ == "task_archiver"]
