"""
Comprehensive test suite for enhanced_websocket_broadcast
Auto-generated for 99%+ coverage achievement
Module: enhanced_websocket_broadcast.py
...
def module_path():
    ...
    return project_root / "enhanced_websocket_broadcast.py"
...
def module_under_test(module_path):
    ...
    if not module_path.exists():
        pytest.skip(f"Module not found: enhanced_websocket_broadcast.py")

    try:
        spec = importlib.util.spec_from_file_location("enhanced_websocket_broadcast", module_path)
        if spec is None or spec.loader is None:
            pytest.skip(f"Cannot create spec for module: enhanced_websocket_broadcast.py")

        module = importlib.util.module_from_spec(spec)
        sys.modules["enhanced_websocket_broadcast"] = module
...
class TestModuleClasses:
    ...
    def test_classes_can_be_instantiated(self, module_under_test):
        ...
        public_classes = [(name, cls) for name, cls in classes
                         if not name.startswith('_') and cls.__module__ == "enhanced_websocket_broadcast"]
